...
from .recurrent import RecurrentBaseModel, SynapRecurrentBaseModel
...
class ALIFRNNModel(RecurrentBaseModel, Epropable):
    def __init__(self,
        ...
        #self.rho = torch.nn.Parameter(rho_tensor)
...
class STDPALIFRNNModel(RecurrentBaseModel, Epropable):
    def __init__(self,
                 hidden_size,
                 output_size=1,
                 initializer=None,
    ...
    def forward(self, inputs, n_timestep=None, *, return_internals=False):
        ...
        threshold = n.zeros((batch_size, self.hidden_size))
        spike = self.activation(hidden_state - self.v_th - self.beta * threshold, refractory=refractory.clone().detach())
        spikes = [spike]
        output = spike @ self.w_ho
        ...
        for t in range(n_timestep):
            ...
            # alle activity tensoren detachen

            # Jetzt foreard pass ausrollen

            # hidden_state = self.alpha * hidden_state + spike @ self.w_hh - n.diag_part(
            #    self.w_hh) * spike + x[:, t, :] @ self.w_ih - self.v_th * spike

            if t >= self.refractory_time:
                spike_ref = spikes[t - self.refractory_time]
            else:
                spike_ref = spike
            ...
            # jetzt second hiiden layer


            threshold = self.rho * threshold + spike
            spike = self.activation(hidden_state - self.v_th - self.beta * threshold, refractory=refractory.clone().detach())

            # hidden_state[spike > 0] = 0.01

            output = self.kappa * output + spike @ self.w_ho
            if self.use_readout_bias:
                output += self.b_ho

            # falls lokales target vorhanden, loss nur für t berechnen
            # loss.backward() -> ersetzt tensor.grad() das lokale Lernsignal L_j
            # entsprechend entfält die partielle Ableitung am e_ij
            #
            # default e-prop:
            #
            # (dE/dw_ij) = e_ij * L_j
            # eps_ij = (ds_j/sd_j) * eps_ij + input...
            # e_ij = (dz_j / ds_j) * epsilon_ij
            #
            # e-prop mit time-lokal backprop:
            #
            # (dE/dw_ij) = (dE/ds) * eps_ij   -> ( dE/ds = s.grad() )
            #
            ...
            # refractory[refractory > self.refractory_time] = 0
            refractory = refractory + refractory.gt(0.0).float()
            mask = spike > refractory
            refractory += mask
            refractory[refractory > self.refractory_time] = 0
    ...
    def dht__dht_1(self):
        ...
        def _dht__dht_1(ht_1, *args, **kwargs):
            ...
            psi = self.d_activation(ht_1[:, 0] - self.beta * ht_1[:, 1] - self.v_th)
            psi = n.where(kwargs['ref'] > 0, n.tensor(-.3, dtype=psi.dtype), psi)

            # if kwargs['t'] >= self.refractory_time:
...
class SynapsALIFRNNModel(SynapRecurrentBaseModel, Epropable): # alpha and rho
    def __init__(self,
                 hidden_size,
                 output_size=1,
                 initializer=None,
    ...
    def forward(self, inputs, n_timestep=None, *, return_internals=False):
        ...
        for t in range(n_timestep):
            hidden_state = hidden_state * self._alpha() + spike @ self.w_hh - n.diag_part(
                self.w_hh) * spike + x[:, t, :] @ self.w_ih - self.v_th * spike
            threshold = threshold * self._rho() + spike
            spike = self.activation(hidden_state - self.v_th - self.beta * threshold)
    ...
    def dht__dht_1(self):
        ...
        def _dht__dht_1(ht_1, *args, **kwargs):
            ...
            d = n.stack([n.stack([n.ones_like(psi) * self._alpha(), n.zeros_like(psi)]),
                         n.stack([psi, self._rho() - self.beta * psi])])
            return n.transpose(d, perm=[2, 0, 1, 3])
    ...
    def _alpha(self):
        return  1 / (1 + self.backend.exponential(-self.alpha))

    def _rho(self):
        return  1 / (1 + self.backend.exponential(-self.rho))
