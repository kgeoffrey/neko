...
class STDPEprop(BaseLearningRule):
    def __init__(self,
                 evaluated_model,
                 optimizer='adam',
                 mode='symmetric',
    ...
    def step(self, x, y):
        ...
        for t in range(n_timestep):
            ...
            if t == 0:
                eligibility_vector_Whh_t = n.einsum('bijm,bjm->bim',
                                                    self.dht__dht_1(self.model.initial_state['h'],
                                                                    spikes=self.model.initial_state['z'],
                                                                    ref = self.model.initial_state['ref']),
                                                    eligibility_vector_Whh_t) + self.dht__dWhh(
                    self.model.initial_state['z'])

            if t >= self.model.refractory_time:
                spikes_ = z[:, t - 1, :] - z[:, t - 1 - self.model.refractory_time, :]
                eligibility_vector_Whh_t = n.einsum('bijm,bjm->bim', self.dht__dht_1(h[:, t - 1, :],
                                                                                     spikes=spikes_,
                                                                                     ref= ref[:, t - 1, :]),
                                                    eligibility_vector_Whh_t) + self.dht__dWhh(z[:, t - 1, :])
            ...
            psi_t, coefficient_vector = self.dzt__dht(h[:, t, :], ref = ref[:, t, :])
            #with open('/Users/geoffreykasenbacher/projects/MSc_Thesis/src/encodings/configs/ref', "wb") as dill_file:
            #    dill.dump(ref, dill_file) # ref[:, t, :]
            eligibility_trace_Whh_t = n.einsum('i,bij->bj', coefficient_vector, eligibility_vector_Whh_t)
            eligibility_trace_Wih_t = n.einsum('i,bij->bj', coefficient_vector, eligibility_vector_Wih_t)
...
class SynapsEprop(BaseLearningRule):
    def __init__(self,
                 evaluated_model,
                 optimizer='adam',
                 s_optimizer='adam',
    ...
    def pretraining_initialization(self):
        ...
        # self.synaps_params = [self.model.alpha, self.model.rho]
        self.synaps_params = [self.model.rho, self.model.alpha]
        self.s_optimizer.watch(parameters=self.synaps_params)
    ...
    def step(self, x, y):
        ...
        self.s_optimizer.calculate_gradients_and_optimize(model_objective)
        self.alpha = self.model.alpha.detach()
        self.rho = self.model.rho.detach()
